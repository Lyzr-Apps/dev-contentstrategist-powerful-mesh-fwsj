...
/**
 * POST /api/agent — Submit async task, return task_id immediately
 */
export async function POST(request: NextRequest) {
  try {
    ...
    // Return task_id immediately — client will poll via GET
    return NextResponse.json({
      task_id,
      agent_id,
      user_id: finalUserId,
...
/**
 * GET /api/agent?task_id=xxx — Single poll proxy (API key protected server-side)
 *
 * Returns:
 *   { status: 'processing' }                          — still running
 *   { status: 'completed', success: true, response, module_outputs, ... }  — done
 *   { status: 'failed', success: false, error, ... }  — agent failed
 */
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Still processing — return immediately so client can schedule next poll
    ...
    // Task completed — parse through envelope extraction + parseLLMJson + normalizeResponse
    ...
    const errorMsg = error instanceof Error ? error.message : 'Network error during poll'
    return NextResponse.json(
      {
        success: false,
        status: 'failed',
