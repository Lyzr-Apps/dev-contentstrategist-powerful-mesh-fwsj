...
export async function callAIAgent(
  ...
  try {
    ...
    // If submit itself failed (validation, API key, upstream error) return as-is
    if (submitData.success === false || !submitData.task_id) {
      return submitData
    }

    const { task_id, user_id, session_id } = submitData

    // 2. Poll GET /api/agent?task_id=... with adaptive backoff from CSR
    ...
    while (Date.now() - startTime < POLL_TIMEOUT_MS) {
      ...
      const pollRes = await fetchWrapper(`/api/agent?task_id=${task_id}`)
